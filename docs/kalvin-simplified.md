...
**Status:** Draft — normative. This document is the single normative definition of Kalvin and supersedes `kalvin-algebra.md` and `kalvin-for-agents.md`.
...
A strategy must bound witnessed runs and traversal. Suitable policies forbid expand-after-contract of the same witness, and adopt no-revisit for traversal: each consumed correspondence signature is used at most once during a slot walk.
...
The final state overlaps the goal's excess and is absorbed as the composed correspondence:

```text
w:[a,l,l]
```

with acquisition depth 3, the three edges crossed.
...
The terminal state is absorbed into memory as a composed correspondence, with acquisition depth equal to the edges crossed, which the main derivation may then consume.
...
Absorbed klines carry their recorded depths; consuming one composes its depth with the current one.
