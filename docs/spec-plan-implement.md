...
1. **Downward references only.** Origin never references specs/plans. Specs may reference origin. Plans reference specs and origin.
...
5. **No file names in specs.** No spec IDs in origin. Code locations belong in plans only.
...
- **Test Matrix** — numbered criteria with origin references (`KL-1`, `MOD-12`).
...
- Origin section → spec criterion → plan task → test function.
- No content duplicated across layers.
- No broken references.

### Step 4 — Commit

Stage and commit all grilling documentation updates (specs, plans, origin changes). **Ask the user for explicit confirmation before committing.**
