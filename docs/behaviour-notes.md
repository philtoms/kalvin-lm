...
- Curriculum-driven: the CLI takes a curriculum markdown file or a raw `.ks` (`.ks` suffix dispatches); each lesson's kscript runs through a shared engine, state persisting across lessons; the label map accumulates the cumulative source.
