...
**Status:** superseded by [ADR-0006 — `op` records the structural state, not the written token](./0006-op-is-structural-state-not-token.md). The undersign-is-S3 reclassification is absorbed and generalised there: every `op` value is a structural state, not a token, and the four token-derived states take past-participle names.
