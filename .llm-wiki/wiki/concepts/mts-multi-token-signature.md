...
the _signature string_, distinct from any CANONICALZES decomposition a script
declares for that signature via a block — a CANONICALZES scope's nodes are the
...
- [[concepts/relational-tokens]] — CANONICALZES declares an intent that may use an MTS signature
