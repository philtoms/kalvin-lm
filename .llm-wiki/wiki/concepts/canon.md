...
Distinct from the CANONICALZES relational token (see [[concepts/relational-tokens]]):
`=>` declares an _intent_ to compose, and a CANONICALZES statement need not
...
- [[concepts/relational-tokens]] — CANONICALZES declares intent, may not produce a Canon
