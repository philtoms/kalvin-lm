---
name: dialogue-dev
description: Drives the dialogue harness (dev/dialogue/harness.py + src/kalvin/rationaliser.py) — the synchronous, non-judging compile→feed→present loop — to tune Kalvin's rationaliser against .ks scripts, and to author scripts that test rationaliser theory or bring out new behaviour. Use when the user says "/dialogue-dev".
---

# Dialogue Harness

Tune **Kalvin's rationaliser** by running a script
through the dialogue harness, reading what the rationaliser actually did from the
trace + the grounded/work_list tails, changing the rationaliser so it does
better, and re-running. The rationaliser is the target of the work; the
...
- **trace** - What the rationaliser did (asks, proposals, groundings)
- **grounded** — what the rationaliser knows (identities, canons, relationships).
- **work_list (pending at end of run)** — what the rationaliser was still working on
  ...
  klines_ (something that should have grounded but the rationaliser had no
...
Each lesson's kscript runs through a shared rationaliser (state persists across
lessons), and the trace is presented.

```

The harness compiles each lesson's kscript, feeds each compiled entry to the rationaliser
...
2. **Run the script.** Read grounded + work_list. Form an
   **exploratory expectation** from the annotation + what has grounded —
   ...
   different signature) is a finding that the rationaliser is on the right track, not a
   failure. Then **report** what you found: did the rationaliser meet the expectation, how
   was it out, what went wrong — feeding the next step.
   Judge the decoded prose honestly: if the rationaliser's proposal is semantic
   gibberish, say so — a near-match is only "on the right track" when
   its decoded prose actually means something.
3. **Diagnose: rationaliser or script?** (See below. Rationaliser first.)
4. **Edit** the suspect (rationaliser code, or — only when confident — the
...
## Diagnose: rationaliser or script?

**Rationaliser first — always.** This is the target work. When a trace stalls,
a kline vanishes, or grounded/work_list diverge from the trainer
expectation, suspect the rationaliser and dig there until the rationaliser path is
genuinely exhausted.
...
advances the use case for the rationaliser — it is not a workaround for
rationaliser bugs. Typical retreats:
...
- **Prime-before-test ordering.** A question fed before the prime's
  ...
  the `.ks` so priming precedes questioning, to test whether the rationaliser
...
- **Shake things up.** Once the rationaliser settles on a script, author a new
...
- **Test a theory.** A minimal `.ks` constructed to exercise one rationaliser
...
- **Rationaliser first.** Suspect the rationaliser before the script; edit `.ks`
...
- **The rationaliser speaks in semantic predicates** (`is_identity`,
...
| Artefact        | Path                          | Role                                                     |
| --------------- | ----------------------------- | -------------------------------------------------------- |
| Rationaliser          | `src/kalvin/rationaliser.py`        | The fork under tune                                      |
...
- **Stop** when grounded + work_list match the trainer expectation and
  the suite passes.
- **Ask the user** only on a genuine rationaliser-semantics fork (e.g.
  "should a `==` goal require compositional operands, or just two
  grounded values?"). Do not ask permission to treat rationaliser behaviour as
