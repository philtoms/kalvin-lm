...
class FileDialog(ModalScreen):
    ...
    CSS = """
    FileDialog, LoadScriptDialog, SaveStateDialog, LoadStateDialog {
        align: center middle;
    }
    ...
    FileDialog .filename-row, SaveStateDialog .filename-row {
        height: auto;
        margin-bottom: 1;
    }

    FileDialog .filename-input, SaveStateDialog .filename-input {
        width: 1fr;
    }
    """
    ...
    def _compose_header(self) -> ComposeResult:
        ...
        with Horizontal(classes="mount-points"):
            for path, label in self.MOUNT_POINTS:
                yield Button(label, id=f"mount-{label.lower().replace(' ', '-')}")
    ...
    def _handle_mount_button(self, button_id: str) -> bool:
        ...
        if button_id and button_id.startswith("mount-"):
            for path, label in self.MOUNT_POINTS:
                expected_id = f"mount-{label.lower().replace(' ', '-')}"
...
class LoadScriptDialog(FileDialog):
    """Modal dialog for loading .ks script files."""

    def __init__(
        self,
        title: str = "Load KScript",
        initial_path: str = "data/scripts",
    ) -> None:
        super().__init__(title, initial_path, file_type=".ks")
    ...
    def compose(self) -> ComposeResult:
        with Container():
            ...
            yield from self._compose_footer("Load")
...
class SaveStateDialog(FileDialog):
    """Modal dialog for saving Agent state."""

    def __init__(
        self,
        title: str = "Save Agent State",
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "cancel-btn":
            ...
            if filename:
                # Add .bin extension if no extension present
...
class LoadStateDialog(FileDialog):
    """Modal dialog for loading Agent state."""

    def __init__(
        self,
        title: str = "Load Agent State",
        initial_path: str = "data",
    ) -> None:
        super().__init__(title, initial_path, file_type=None)  # Allow .bin or .json
    ...
    def on_directory_tree_file_selected(self, event: DirectoryTree.FileSelected) -> None:
        """Handle file selection - accept .bin or .json files."""
        path_str = str(event.path)
        if path_str.endswith(".bin") or path_str.endswith(".json"):
            self._selected_path = event.path
            self.query_one("#current-path", Label).update(str(event.path))
