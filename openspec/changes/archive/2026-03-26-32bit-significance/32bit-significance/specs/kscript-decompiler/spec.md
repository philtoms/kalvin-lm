...
- **WHEN** KLine has bit 63 set and node exists at S1 level
...
#### Scenario: S1 undersign detection

- **WHEN** KLine has bit 63 set and node does not exist at S1 level
- **THEN** decompiler identifies as S1 undersign and emits `=` operator

#### Scenario: S4 identity detection

- **WHEN** KLine has no significance bits set (bits 32-63 clear)
- **THEN** decompiler identifies as S4 identity and emits `=` operator
