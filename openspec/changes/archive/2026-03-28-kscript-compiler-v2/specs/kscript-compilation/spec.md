# KScript Compilation Specification
...
- S1 (undersign): unidirectional signature links
- S2 (canonize): multi-node composition
- S3 (connotate): single-node annotation with entity emission
- S4 (identity): unidirectional with entity emission
...
- **WHEN** compiler processes `A = B`
- **THEN** entry {A|S1: B} emitted with S1 significance

#### Scenario: Identity significance

- **WHEN** compiler processes `A = B`
- **THEN** entry {A|S4: None} emitted with S4 significance
