...
class KLine:
    ...
    def is_identity(self) -> bool:
        """Check if this is a identity KLine (nodes is None)."""
        return self._nodes is None

    def is_signed(self) -> bool:
        """Check if this is a signed KLine (nodes is a single KNode)."""
        return isinstance(self._nodes, int)

    def is_canonized(self) -> bool:
        """Check if this is a canonized KLine (nodes is a list of KNodes)."""
        return isinstance(self._nodes, list)

    @property
    def nodes(self) -> list[KNode]:
    ...
    def contains(self, signature: KSig) -> bool:
        ...
        if self._nodes is None:
            return False
        if isinstance(self._nodes, int):
            return self._nodes == signature
        return signature in self._nodes
