...
**Auto-Tune**:
The project's experimental loop for tuning Kalvin's rationalisation behaviour. An LLM coding agent runs repeated sessions against a curriculum, observes how the reactor/cogitator/rationaliser actually behave, edits the significance-model code (`expand()`, `significance.py`, the rationaliser) together with the owning spec, and re-runs to confirm. The thing being tuned is Kalvin's **significance model** (the code and specs that define how significance and proposals are produced) — not Kalvin's **trained memory** (the persisted `.bin` artifact, which auto-tune never edits directly; it is only snapshotted/restored for before-after comparison). Incidental codebase goals are also valid.
_Avoid_: tuning session (ambiguous with training session), auto-train (it's not training), auto-tune supervisor (the CLI includes the full auto-tune tool, not just the supervisor), and reading "not Kalvin's model" as "not the significance model" — the significance model _is_ the target; the trained-memory artifact is not.
