...
The significance a kline's structure **claims** — an S-level (the same **S1**–**S4** as **Rational Significance**) derived from the signature–nodes relationship alone, without model traversal. The claim is coverage: **S4** — no nodes (**Unknown**); **S1** — the signature covers its nodes exactly (**Identity**, **Canon**); **S2** — at least one node is covered by the signature (underfit, overfit, under+over, **Denotation**); **S3** — no node is covered (**Connotation**, no-fit). The rules are independent of node count and of the **Relational Tokens** that compile the shape. A claim that **Cogitation** measures against what Kalvin actually holds.
...
- underfit (`{ABC: [A, C]}`): signature covers every node and more — attracts kline expansion. Claims **S2**
- overfit (`{AB: [A, B, C]}`): signature covers some nodes, not all — attracts kline contraction. Claims **S2**
- under+over (`{ABC: [B, C, D]}`): signature covers some nodes while claiming others — attracts both. Claims **S2**
- denote (`{AB: [B]}`): the compound signature covers its node — attracts association. Claims **S2**
- connote (`{A: [B]}`): no coverage — attracts association. Claims **S3**
- no-fit (`{AB: [C, D]}`): no coverage — attracts kline substitution. Claims **S3**
...
- **S2** Relates but diverges: an unratified misfit with coverage, an active mismatch. _I infer this, but it does not yet fit._
...
**Token ID**:
A value produced by the tokenizer: `(word_bit << 32) | bpe_token_id`. The **word word** (upper 32 bits) carries one bit per distinct word — assigned on a first-encountered basis at bits 0–30 (the word size is 31; a 32nd distinct word is a system error), with bit 31 reserved for **ASK_BPE_TOKEN**. A multi-subword word (one the BPE tokenizer splits) is still one word and one bit; its subword token ids OR into the lower half. A compound (an MTS signature, or a DENOTES concatenation signature) composes no bit of its own — it is the OR-reduction of its component words' values.
...
- **CONNOTES** (`>` / `<`) — 1:1 `{A: [B]}` (subjectively, _A is a kind of B_); `A < B` ⇒ `{B: [A]}` — the same connotation written from the other side (_B is a kind of A_; the identifier reverses to match the reading direction, so `A < B` compiles identically to `B > A`). Self-reference (`A > A`) collapses to IDENTITY `{A: [A]}`.
- **DENOTES** (`=`) — 1:1 with a compound signature. `A = B` ⇒ `{AB: [B]}`: the signature is the compound of both operands and the node is the denoted value (objectively, _A is a B_; the compound `AB` denotes `B`). The compound contains its node (underfit), so a denotation claims **S2** — the connotation's disjoint `{A: [B]}` claims **S3**. Self-denote (`A = A`) collapses to IDENTITY.
